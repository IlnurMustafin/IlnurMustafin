# Ильнур Мустафин

## Курс "Разработчик нейросетей"
**АНО "Цифротех"**  
Статус: 48/52 уроков завершено 

---

## 📌 О себе

Привет! Я прохожу курс по разработке нейросетей и делюсь своими проектами.  
Здесь собраны практические работы, выполненные в процессе обучения.

---

## 🛠 Технологический стек

![Python](https://img.shields.io/badge/Python-3.9+-blue)
![PyTorch](https://img.shields.io/badge/PyTorch-2.0+-red)
![TensorFlow](https://img.shields.io/badge/TensorFlow-2.13+-orange)
![scikit-learn](https://img.shields.io/badge/scikit--learn-1.3+-green)
![Pandas](https://img.shields.io/badge/Pandas-2.0+-blue)
![NumPy](https://img.shields.io/badge/NumPy-1.24+-lightblue)
![Matplotlib](https://img.shields.io/badge/Matplotlib-3.7+-yellow)

---
<!--
**IlnurMustafin/IlnurMustafin** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
